...
export async function generateBlurb(
  ...
  try {
    ...
    try {
      ...
      systemPrompt =
        typeof configPrompt === "string" ? configPrompt : DEFAULT_SYSTEM_PROMPT;
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
    } catch (_error: unknown) {
      console.warn(
        "Failed to get prompt from Edge Config, using default prompt",
      );
...
Weather: ${weather?.city || "Unknown location"}, ${
        weather?.temperature || "unknown"
      }°F, ${weather?.condition || "unknown condition"}, Humidity: ${
        weather?.mean_humidity || "unknown"
      }%

${
  latestArticle
    ? `Latest article read: "${latestArticle.title}"
Excerpt: "${latestArticle.excerpt || "No excerpt available"}"`
    : "No recent articles"
}

${
  lastTrack
    ? `Last played music: "${lastTrack.title}" by ${lastTrack.artist}`
    : "No recent music"
}
...
Create a natural-sounding, casual sentence (max 80 tokens) that mentions these activities.`,
    ...
    return `Jason is currently in ${
      profile.weather?.city || "Atlanta"
    }, enjoying the day and working on interesting projects.`;
  }
}
