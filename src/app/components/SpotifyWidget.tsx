...
export default async function SpotifyWidget() {
  ...
  if (!trackData) {
    return (
      <div className="spotify-widget frost-panel p-5 text-[var(--u-ink-muted)]">
        <p>Music data unavailable</p>
        {spotifyData?.lastUpdated && (
          <p className="mt-2 font-mono text-[0.68rem] uppercase tracking-wider text-[var(--u-ink-muted)]">
  ...
  return (
    <div className="spotify-widget frost-panel p-5">
      ...
      <div className="mt-2 flex items-center">
        ...
        <div className="flex-1">
          <p className="text-lg font-medium text-[var(--u-ink-strong)]">
            {trackData.title}
          </p>
          <p className="mt-1 text-sm text-[var(--u-ink-muted)]">
            by {trackData.artist}
          </p>
          {trackData.trackUrl && (
            <a
              ...
              className="mt-2 inline-block text-xs font-medium text-[var(--u-accent)] transition-colors hover:text-[var(--u-accent-strong)]"
            >
              Listen on Spotify →
            </a>
          )}
      ...
      {spotifyData?.lastUpdated && (
        <p className="mt-4 font-mono text-[0.68rem] uppercase tracking-wider text-[var(--u-ink-muted)]">
          Updated {formatUpdatedAt(spotifyData.lastUpdated)}
        </p>
      )}
    </div>
